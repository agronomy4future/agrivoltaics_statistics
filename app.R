...
build_formula <- function(v) {
  ...
  SITE   <- v$site
  BLOCK  <- v$block
  ROW1   <- NULL; if (has("row"))      ROW1   <- v$row
  SEASON <- NULL; if (has("season"))   SEASON <- v$season
  GENO   <- NULL; if (has("genotype")) GENO   <- v$genotype

  block_dep   <- isTRUE(v$block_type == "dependent")
  has_rep     <- isTRUE(v$has_replicates)
  is_peren    <- isTRUE(v$crop_type == "perennial")
  n_season    <- 0L
  if (!is.null(v$n_season)) n_season <- as.integer(v$n_season)
  ...
  case_num <- 1L
  if      (!has_geno && !has_row && !has_season) { case_num <- 1L
  } else if (!has_geno &&  has_row && !has_season) { case_num <- 2L
  } else if (!has_geno && !has_row &&  has_season) { case_num <- 3L
  } else if (!has_geno &&  has_row &&  has_season) { case_num <- 4L
  } else if ( has_geno && !has_row && !has_season) { case_num <- 5L
  } else if ( has_geno &&  has_row && !has_season) { case_num <- 6L
  } else if ( has_geno && !has_row &&  has_season) { case_num <- 7L
  } else { case_num <- 8L }

  # ── fixed effects
  fixed <- SITE
  if (case_num == 2L) {
    fixed <- c(SITE, ROW1, paste0(SITE, ":", ROW1))
  } else if (case_num == 3L) {
    fixed <- c(SEASON, SITE, paste0(SEASON, ":", SITE))
  } else if (case_num == 4L) {
    fixed <- c(SEASON, SITE, ROW1,
               paste0(SEASON, ":", SITE),
               paste0(SEASON, ":", ROW1),
               paste0(SEASON, ":", SITE, ":", ROW1))
  } else if (case_num == 5L) {
    fixed <- c(SITE, GENO, paste0(SITE, ":", GENO))
  } else if (case_num == 6L) {
    fixed <- paste(SITE, GENO, ROW1, sep=" * ")
  } else if (case_num == 7L) {
    fixed <- paste(SITE, GENO, SEASON, sep=" * ")
  } else if (case_num == 8L) {
    fixed <- paste(SITE, GENO, SEASON, ROW1, sep=" * ")
  }
  ...
  if (!has_rep) {
    ...
    random <- sprintf("(1|%s:%s)", SITE, BLOCK)
  } else if (case_num == 2L) {
    if (block_dep) {
      random <- c(sprintf("(1|%s)", BLOCK), sprintf("(1|%s:%s:%s)", SITE, BLOCK, ROW1))
    } else {
      random <- c(sprintf("(1|%s)", BLOCK), sprintf("(1|%s:%s)", BLOCK, ROW1))
    }
  } else if (case_num == 3L) {
    if (block_dep) {
      if (peren_valid) {
        random <- c(sprintf("(1|%s)", BLOCK), sprintf("(1|%s:%s:%s)", SITE, BLOCK, SEASON))
      } else {
        random <- sprintf("(1|%s)", BLOCK)
      }
    } else {
      random <- sprintf("(1|%s:%s)", SITE, BLOCK)
    }
    ...
    if (block_dep) {
      if (peren_valid) {
        random <- c(sprintf("(1|%s)", BLOCK), sprintf("(1|%s:%s:%s:%s)", SITE, BLOCK, ROW1, SEASON))
      } else {
        random <- c(sprintf("(1|%s)", BLOCK), sprintf("(1|%s:%s:%s)", SITE, BLOCK, ROW1))
      }
    } else {
      if (peren_valid) {
        random <- c(sprintf("(1|%s:%s)", SITE, BLOCK), sprintf("(1|%s:%s:%s:%s)", SITE, BLOCK, ROW1, SEASON))
      } else {
        random <- c(sprintf("(1|%s:%s)", SITE, BLOCK), sprintf("(1|%s:%s)", BLOCK, ROW1))
      }
    }
  } else if (case_num == 5L) {
    if (block_dep) {
  ...
  use_lm   <- !has_rep
  model_fn <- "lmer"
  if (use_lm) model_fn <- "lm"

  fixed_str  <- paste(fixed,  collapse=" + ")
  random_str <- paste(random, collapse=" + ")
  formula_str <- sprintf("%s ~ %s + %s", Y, fixed_str, random_str)

  warnings <- character(0)
  if (!has_rep && case_num == 1L && !block_dep) {
    warnings <- c(warnings,
      "Warning: Even though block is independent, Block is used as fixed effect due to confounding with Site.")
  }
  if (has_season && is_peren && n_season < 4L) {
    warnings <- c(warnings,
      sprintf("Warning: Perennial selected but only %d season(s) detected. Minimum 4 required. Annual model applied.", n_season))
  }
  if (case_num >= 6L) {
    warnings <- c(warnings,
      "Warning: Plot variable not used - including Plot would absorb Genotype variance into random effects.")
  }
...
server <- function(input, output, session) {
  ...
  output$subset_ui <- renderUI({
    ...
    tagList(
      fluidRow(
        column(4,
          selectInput("subset_col", "Filter Column",
                      choices = c("— none —" = "none", setNames(cols, cols)))
        ),
        column(8, uiOutput("subset_val_ui"))
      ),
  ...
  output$subset_val_ui <- renderUI({
    ...
    vals <- sort(unique(as.character(rv$df[[input$subset_col]])))
    selectInput("subset_vals", 
                label    = paste("Select values to keep:"),
                choices  = vals,
                selected = vals,
                multiple = TRUE)
  })
  ...
  filtered_df <- reactive({
    req(rv$df)
    df <- rv$df
    if (!is.null(input$subset_col) && input$subset_col != "none" &&
        !is.null(input$subset_vals) && length(input$subset_vals) > 0)
      df <- df[as.character(df[[input$subset_col]]) %in% input$subset_vals, ]
    df
  })
  ...
  output$subset_preview_text <- renderUI({
    ...
    if (n_orig == n_filt) return(NULL)
    div(style = "margin-top:6px; padding:8px 12px; background:#e8f4f8;
                 border-radius:6px; font-size:13px; color:#2c2c2c;",
        icon("filter"),
  ...
  get_vars <- reactive({
    ...
    geno_val <- "none"
    if (!is.null(input$sel_genotype)) { geno_val <- input$sel_genotype }
    row_val <- "none"
    if (!is.null(input$sel_row))      { row_val  <- input$sel_row }
    season_val <- "none"
    if (!is.null(input$sel_season))   { season_val <- input$sel_season }
    crop_val <- "annual"
    if (!is.null(input$crop_type))    { crop_val <- input$crop_type }
  ...
  output$formula_preview <- renderUI({
    ...
    fn_prefix <- "lmer("
    if (result$model_fn=="lm") { fn_prefix <- "lm(" }
  ...
  analysis_result <- eventReactive(input$btn_run, {
    ...
    if (v$genotype != "none") { fac_cols <- c(fac_cols, v$genotype) }
    if (v$row      != "none") { fac_cols <- c(fac_cols, v$row) }
    if (v$season   != "none") { fac_cols <- c(fac_cols, v$season) }
    fac_cols <- unique(fac_cols)
    for (col in fac_cols)
      if (col %in% colnames(df)) df[[col]] <- as.factor(df[[col]])
    df[[v$y]] <- as.numeric(df[[v$y]])
    formula_obj <- as.formula(result$formula_str)
    model <- tryCatch(lmer(formula_obj, data=df, REML=TRUE), error=function(e) stop(paste("lmer() failed:", e$message)))
    if (result$model_fn=="lm") {
      model <- tryCatch(lm(formula_obj, data=df), error=function(e) stop(paste("lm() failed:", e$message)))
    }
    vc_tbl <- NULL
    if (result$model_fn=="lmer") {
      vc <- as.data.frame(VarCorr(model))
      vc_total <- sum(vc$vcov)
      vc$pct <- round(vc$vcov/vc_total*100, 2)
      vc$vcov <- round(vc$vcov, 3)
      vc_tbl <- setNames(vc[,c("grp","vcov","pct")], c("Groups","Variance","% of Total"))
    }
    anova_tbl <- as.data.frame(anova(model))
    if (result$model_fn=="lmer") { anova_tbl <- as.data.frame(anova(model, type=3)) }
    anova_tbl <- round(anova_tbl, 4)
    p_col <- NULL
    if ("Pr(>F)" %in% colnames(anova_tbl)) { p_col <- "Pr(>F)" }
    if (!is.null(p_col))
    ...
    default_term <- v$site
    if (length(interaction_terms) > 0) { default_term <- interaction_terms[which.max(nchar(interaction_terms))] }
  ...
  output$posthoc_selector <- renderUI({
    ...
    if (default %in% terms) { selected <- default }
  ...
  output$plot_anova <- renderPlot({
    ...
    f_col <- NULL
    if ("F value" %in% colnames(tbl)) { f_col <- "F value" } else if ("F" %in% colnames(tbl)) { f_col <- "F" }
    req(!is.null(f_col))
    tbl$Significant <- TRUE
    if ("Sig" %in% colnames(tbl)) { tbl$Significant <- tbl$Sig!="ns" }
  ...
  output$r_code <- renderText({
    ...
    pt <- analysis_result()$default_term
    if (!is.null(posthoc_result())) {
      pt <- posthoc_result()$posthoc_term
    }

    warn_lines <- ""
    if (length(warns) > 0) {
      warn_lines <- paste0(paste0("# ", warns, collapse="\n"), "\n\n")
    }

    geno_line <- ""
    if (v$genotype != "none") {
      geno_line <- paste0("df$", v$genotype, " <- as.factor(df$", v$genotype, ")\n")
    }
    row_line <- ""
    if (v$row != "none") {
      row_line <- paste0("df$", v$row, " <- as.factor(df$", v$row, ")\n")
    }
    season_line <- ""
    if (v$season != "none") {
      season_line <- paste0("df$", v$season, " <- as.factor(df$", v$season, ")\n")
    }
    vc_line <- ""
    if (fn == "lmer") {
      vc_line <- "# Variance components\nprint(VarCorr(model), comp='Variance')\n\n"
    }

    pt_first <- strsplit(pt, ":")[[1]][1]
    ...
    paste0(
      ...
      paste0("       aes(x=reorder(", pt_first, ", emmean), y=emmean)) +\n"),
