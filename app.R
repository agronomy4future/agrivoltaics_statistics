...
server <- function(input, output, session) {
  ...
  output$subset_ui <- renderUI({
    ...
    tagList(
      fluidRow(
        column(4, selectInput("subset_col", "Filter Column",
  ...
  output$subset_preview_text <- renderUI({
    ...
    div(style="margin-top:6px;padding:8px 12px;background:#e8f4f8;border-radius:6px;font-size:13px;color:#2c2c2c;",
        icon("filter"),
  ...
  analysis_result <- eventReactive(input$btn_run, {
    ...
    if (length(interaction_terms) > 0) {
      default_term <- interaction_terms[which.max(nchar(interaction_terms))]
    }
  ...
  output$posthoc_selector <- renderUI({
    ...
    selectInput("sel_posthoc_term", label="Select Post-hoc Term:",
                choices=terms, selected=selected)
  })

  posthoc_result <- reactive({
    req(analysis_result()); req(input$sel_posthoc_term)
  ...
  output$plot_posthoc <- renderPlot({
    req(analysis_result()); req(posthoc_result()$cld_tbl)
    tbl <- posthoc_result()$cld_tbl
    tbl <- tbl[order(tbl$emmean, decreasing=TRUE),]
    v   <- analysis_result()$v
